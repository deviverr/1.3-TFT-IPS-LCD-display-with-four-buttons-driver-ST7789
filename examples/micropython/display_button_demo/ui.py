...
def draw_char(display, x, y, ch, color, scale=2):
    pattern = FONT_5X7.get(ch, FONT_5X7[" "])
    # FONT_5X7 stores 5-bit rows; render rows first to avoid transposed glyphs.
    for row in range(5):
        bits = pattern[row]
        for col in range(5):
            if bits & (1 << (4 - col)):
...
def draw_text(display, x, y, text, color, scale=2):
    cursor_x = x
    for ch in text.upper():
        draw_char(display, cursor_x, y, ch, color, scale)
        cursor_x += 6 * scale


def draw_button_hint(display, y, text, fg, bg):
    display.fill_rect(0, y, 240, 24, bg)
    draw_text(display, 6, y + 4, text, fg, scale=2)
